  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('sysuser.roleId')">
        <template slot-scope="{row}">
          <span>{{ row.roleIdCn|arrayToString() }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('sysuser.orgId')">
        <template slot-scope="{row}">
          <span>{{ row.orgIdCn|arrayToString() }}</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="90px" style="width: 100%">
        ...
        <el-form-item :label="$t('sysuser.roleId')" prop="roleId">
          <el-select v-model="temp.roleId" multiple :placeholder="$t('common.choose')" clearable class="filter-item" style="width: 100%">
        ...
        <el-form-item :label="$t('sysuser.orgId')" prop="orgId">
            <el-select ref="selectOrgId" multiple v-model="temp.orgId" :placeholder="$t('common.choose')" style="width: 100%" clearable>
                    <template v-for="item in orgIdOptions">
                       <el-option :key="item.value" :label="item.title" :value="item.value" hidden/>
                       <template v-for="childrenItem in item.children">
                             <el-option :key="childrenItem.value" :label="childrenItem.title" :value="childrenItem.value" hidden/>
                             <template v-for="finalItem in childrenItem.children">
                                <el-option :key="finalItem.value" :label="finalItem.title" :value="finalItem.value" hidden></el-option>
                                <el-option v-for="ffinalItem in finalItem.children" :key="'f-'+ffinalItem.value" :label="ffinalItem.title" :value="ffinalItem.value" hidden/>
                             </template>                     
...
export default {
  ...
  methods: {
    ...
    handleNodeClickOrgId(data) {
      ...
      if(this.temp.orgId)
        if(this.temp.orgId.indexOf(data.id)==-1)
        this.temp.orgId.push(data.id)
      else{
        this.temp.orgId=[data.id]
      }
      console.log(this.temp.orgId)
